	...
	if(addressObj.length){
		...
		for(var i=0;i<addressObj.length;i++){
			...
			if(addressObj[i].data.state==1){
				defaultHtml = '<div class="address_pox" style="position:relative" data_id="'+addressObj[i].id+'">'+
								'<div class="left">'+
		...
		$('.add_your_address').show();
		$('.add_your_address').css({backgroundImage:'url(img/add_address2.png)',backgroundRepeat:'no-repeat',backgroundSize:'15rem 11.875rem',backgroundPosition:'center center'})
	}
	if(!$('.default_address .address_pox').length){
		$('.default_address').html('<div class="address_pox"><a href="add_address.html" class="text-center" style="height:5.625rem;line-height:5.625rem;display:block;font-family: PingFangSC-Thin;font-size: 0.875rem;letter-spacing: 0.14px;color: #868191;">请选择默认地址哦～</a></div>');
	}
})
